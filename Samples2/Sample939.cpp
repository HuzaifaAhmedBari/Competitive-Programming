...
int main()
{
    ...
    cin>>q;
    vll trie;
    trie.push_back(vl(2,-1));
    function<void(ll,ll)>build = [&](ll x,ll j) {
        ll node = 0;
        forr(31,0)
        {
            ll cur = (x>>r)&1;
    ...
    function<pii(ll)>ans = [&](ll x) {
        ll res = 0, node = 0;
        forr(31,0)
        {
            ll cur = (x>>r)&1;
            ll doosra = 1-cur;
            if(trie[node][doosra]!=-1)
            {
                node = trie[node][doosra];
                res |= (1ll<<r);
            }
            else
            {
