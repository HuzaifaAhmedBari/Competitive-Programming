...
int main()
{
    ...
    while(t--)
    {
        ...
        ll leaf = 0;
        ll c=0;
        function<ll(ll,ll)> ret = [&](ll u,ll p){
            if(adj[u].size()==1 and adj[u].front()==p)
                return 1LL;
            return 0LL;
        };
        function<ll(ll)> cet = [&](ll u) {
            ll cnt=0;
            for(ll v:adj[u])
                cnt += ret(v,u);
            return cnt;
        ...
        function<void(ll,ll)> dfs = [&](ll u,ll p) {
            visited[u] = true;
            if(adj[u].size()==1 and adj[u].front()!=idx and adj[u].front()==p)
            {
                count++;
                return;
            }
