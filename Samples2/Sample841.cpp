...
int main()
{
    ...
    function<ll(ll,ll,ll,ll)> ans = [&](ll idx, ll l,ll r, ll val) {
        if(l==r)
        {
            update(1,0,n-1,l,tree[idx]-val);
            return l+1;
        }
        ll mid = (l+r)/2;
        if(tree[idx*2]>=val)
            return ans(idx*2,l,mid,val);
        else if(tree[idx*2+1]>=val)
            return ans(idx*2+1,mid+1,r,val);
        else
            return 0LL;
    };
    build(1,0,n-1);
