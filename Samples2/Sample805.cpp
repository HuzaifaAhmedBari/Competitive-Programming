...
int main()
{
    ...
    function<ll(ll,ll,ll,ll,ll)> result = [&](ll idx,ll l,ll r,ll a,ll b) {
        if(b<l or a>r)
            return LLONG_MAX;
        if(a<=l and b>=r)
            return tree[idx];
        ll mid = (l+r)/2;
        return min(result(idx*2,l,mid,a,b),result(idx*2+1,mid+1,r,a,b));
    };
    build(1,0,n-1);
    while(q--)
    {
