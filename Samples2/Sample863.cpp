...
int main()
{
    ...
    vl every;
    fori(0,n)
        every.push_back(arr[i]);
    vector<tuple<char,ll,ll>> query(q);
    fori(0,q)
    {
        char c;
        ll l,r;
    ...
    sahi(every);
    every.erase(unique(all(every)),every.end());
    function<ll(ll)> getid = [&](ll x) {
        return lower_bound(all(every),x) - every.begin();
    };
    ll m = every.size();
    vl comp(n);
    tree.assign(m*4,0);
    ...
    for(auto [c,l,r] : query)
    {
        if(c=='!')
        {
            ll old = comp[l-1];
            ll newi = getid(r);
            update(1,0,m-1,old,-1);
            update(1,0,m-1,newi,1);
            comp[l-1] = newi;
        }
        else
        {
