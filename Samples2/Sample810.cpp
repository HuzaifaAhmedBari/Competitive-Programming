...
int main()
{
    ...
    function<bool(ll,ll)> can = [&](ll x,ll y){
        return x>=1 and x<=n and y>=1 and y<=m;
    };
    vpii dis = {{1,0},{-1,0},{0,1},{0,-1}};
    pii ans = {1,1};
    while(!q.empty())
    {
        pii z = q.front();
        ...
        fori(0,4)
        {
            pii nx = {z.fs+dis[i].fs,z.sc+dis[i].sc};
            if(can(nx.fs,nx.sc) and not s[nx.fs][nx.sc])
            {
                s[nx.fs][nx.sc] = true;
                q.push(nx);
            }
