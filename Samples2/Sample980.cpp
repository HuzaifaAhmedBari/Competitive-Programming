...
int main()
{
    ...
    ll count=0;
    map<ll,ll> s;
    function<ld(ld,ld,ld,ld)>f = [&](ld x2,ld x1,ld y2, ld y1) {
    ...
    fori(0,m)
    {
        ...
        forj(0,n)
        {
            dist = min(dist,f(arr1[i].fs,arr2[j].fs,arr1[i].sc,arr2[j].sc));
        }
        forj(0,n)
        {
            if(f(arr1[i].fs,arr2[j].fs,arr1[i].sc,arr2[j].sc)==dist)
            {
                s[j]++;
                break;
            }
