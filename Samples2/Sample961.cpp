...
vl parent;
vl cost;
vl dijkstra(ll n, ll source, vector<vector<pair<ll,pii>>> &adj)
{
    vl dist(n, LLONG_MAX);
    cost.assign(n+1, 0);
    parent.assign(n+1, -1);
    ...
    while (!pq.empty())
    {
        ...
        for (auto [v, w] : adj[u])
        {
            ...
            else if(dist[u] + w.fs == dist[v])
            {
                cost[v] = min(w.sc,cost[v]);
            }
        }
    }
    return dist;
...
int main()
{
    ...
    while(1)
    {
        ...
        if(n==0 and m==0)
            re;
        ll u,v,w,c;
        vector<vector<pair<ll,pii>>> adj(n+1);
        fori(0,m)
        {
            cin>>u>>v>>w>>c;
