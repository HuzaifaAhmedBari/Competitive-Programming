...
int main()
{
    ...
    while(t--)
    {
        ...
        cin>>s[0]>>s[1];
        queue<pii> q;
        vbb visited(4,vb(s[0].size()+2,false));
        function<bool(ll,ll)> fine = [&](ll x, ll y) {
            return (x>=0 and x<=s[0].size()-1 and y>=0 and y<=1 and not visited[y][x]);
        };
        ll te = s[0].size()-1;
        bool flag=false;
        ...
        while(not q.empty())
        {
            ...
            for(auto dx:ds)
            {
                if(fine(dx.fs+u.fs,dx.sc+u.sc))
                {
                    visited[u.sc+dx.sc][u.fs+dx.fs] = true;
                    q.push({u.fs+dx.fs+((s[u.sc+dx.sc][u.fs+dx.fs]=='>')? 1:-1),u.sc+dx.sc});
                }
            }
        }
        YES(flag);
