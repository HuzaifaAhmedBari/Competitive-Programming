...
int main()
{
    ...
    forr(n,1)
    {
        while(not s.empty() and arr[s.top()]<=arr[r])
        {
            s.pop();
        }
        nextmax[r] = (s.empty())? n+1 : s.top();
        s.push(r);
    }
    vll next(21,vl(n+1,n+1)), count(21,vl(n+1,0));
    fori(1,n+1)
    {
        next[0][i] = nextmax[i];
        count[0][i] = 1;
    ...
    fork(1,20)
    {
        fori(1,n+1)
        {
            ...
            {
                next[k][i] = n+1;
                count[k][i] = count[k-1][i];
            }
            else
            {
                next[k][i] = next[k-1][reach];
                count[k][i] = count[k-1][i] + count[k-1][reach];
            }
        }
    }
    while(q--)
