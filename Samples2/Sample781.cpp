...
int main()
{
    ...
    function<void(ll,vb&,vll&)>dfs = [&](ll u,vb &visited, vll&adj) {
        visited[u] = true;
        for(ll v:adj[u])
            if(not visited[v])
                dfs(v,visited,adj);
    ...
    dfs(1,visited1,adj);
    dfs(1,visited2,rev);
    auto it1 = find(all(visited1),false);
    auto it2 = find(all(visited2),false);
    if(it1 == visited1.end() and it2 == visited2.end())
    {
        YES(1);
        nl;
        re;
    ...
    if(it1!=visited1.end())
    {
        cout<<1<<" "<<it1-visited1.begin();
    }
    else
    {
        cout<<it2-visited2.begin()<<" "<<1;
    }
    nl;
    return 0;
}