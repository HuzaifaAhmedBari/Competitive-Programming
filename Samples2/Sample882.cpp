...
int main()
{
    ...
    ll t = 1;
    cin>>t;
    ll mnx=LLONG_MAX, mxx=LLONG_MIN, mny=LLONG_MAX, mxy=LLONG_MIN,sumx=0,sumy=0;
    fori(0,t)
    {
        ll a,b;
        cin>>a>>b;
    ...
    ll x = round((mnx+mxx)/double(2));
    ll y = round((mny+mxy)/double(2));
    cout<<max(max(abs(mxx-x),abs(mnx-x)),max(abs(mny-y),abs(mxy-y)));
    nl;
    return 0;
}