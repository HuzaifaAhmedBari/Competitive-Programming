...
int main()
{
    ...
    fori(1,n+1)
    {
        ll found=-1,mx=LLONG_MIN;
        forj(0,leaf.size())
        {
            bool flag=false;
            fork(0,arr.size())
            {
                if(arr[k].count(i) and arr[k].count(leaf[j]))
                {
                    flag=true;
                    break;
                }
            ...
            if(flag)
            {
                ll z = min(dist[i],dist[leaf[j]]);
                ll o = max(dist[i],dist[leaf[j]]);
                if(o-z>mx)
                {
                    mx = o-z;
                    found = leaf[j];
                }
                else if(o-z==mx and leaf[j]>found)
                {
                    found = leaf[j];
                }
            }
            ...
            {
                if(dist[i]+dist[leaf[j]]>mx)
                {
                    mx = dist[i]+dist[leaf[j]];
                    found = leaf[j];
                }
                else if(dist[i]+dist[leaf[j]]==mx)
                {
                    found = leaf[j];
                }
            }
