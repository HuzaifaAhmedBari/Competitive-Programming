...
int main()
{
    ...
    while(t--)
    {
        ...
        vll arr(2,vl(n));
        cin>>arr;
        vll l(2,vl(n,0)), r(2,vl(n,0));
        l[0][0] = r[0][0] = arr[0][0];
        fori(1,n)
        {
            l[0][i] = min(arr[0][i],l[0][i-1]);
            r[0][i] = max(arr[0][i],r[0][i-1]);
        }
        l[1][0] = min(arr[1][0],arr[0][0]);
        r[1][0] = max(arr[0][0],arr[1][0]);
        fori(1,n)
        {
            l[1][i] = max(min(l[0][i],arr[1][i]),min(l[1][i-1],arr[1][i]));
            r[1][i] = min(max(r[0][i],arr[1][i]),max(r[1][i-1],arr[1][i]));
        }
        cout<<l[1][n-1]*((2*n)+1-r[1][n-1]);
        nl;
    }
    return 0;
}