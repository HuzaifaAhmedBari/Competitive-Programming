...
private:
    ...
    ll query(ll ql, ll qr, ll idx, ll l, ll r, ll x) {
        if(qr < l || r < ql)
            return -1;
        if(not(tree[idx].val&(1LL<<x)))
            return -1LL;
        if(l==r)
            return l;
        ll mid = (l + r) / 2;
        ll a = query(ql,qr,idx*2,l,mid,x);
        if(a!=-1)
            return a;
        return query(ql,qr,idx*2+1,mid+1,r,x);
    }
...
public:
    ...
    ll query(ll l, ll r, ll x) { return query(l, r, 1, 0, n - 1, x); }
};
int main()
{
    io;
    ...
    fori(0,n-1)
    {
        if(arr[i]==arr[i+1])
        {
            ll idx = st.query(i+1,n+1,((arr[i+1]==1)?2:1));
            ans+=(idx-(i+1));
            swap(arr[i+1],arr[idx]);
            st.update(i+1,arr[i+1]);
