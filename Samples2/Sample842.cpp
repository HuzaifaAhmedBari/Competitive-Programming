...
struct Node {
    ll sum,pre,suf,best;
    Node(ll x=0) : sum(x),pre(max(0LL,x)),suf(max(0LL,x)),best(max(0LL,x)) {}
    Node(ll x,ll y,ll z,ll b) : sum(x), pre(max(0LL,y)), suf(max(0LL,z)), best(max(0LL,b)) {}
    Node operator+(const Node &other) const {
        return Node(sum+other.sum,max(pre,sum+other.pre),max(other.suf,suf+other.sum),max({best,other.best,suf+other.pre}));
    }
};
int main()
{
    ...
    function<void(ll,ll,ll,ll,ll)> update = [&](ll idx, ll l, ll r, ll pos, ll val) {
        if(l==r)
        {
            tree[idx] = val;
            return;
    ...
    function<Node(ll,ll,ll,ll,ll)> ans = [&](ll idx, ll l,ll r,ll a,ll b) {
        if(b<l or a>r)
            return Node();
        if(a<=l and b>=r)
            return tree[idx];
