...
int main()
{
    ...
    queue<pii> q;
    q.push({0,0});
    function<bool(ll,ll)> issafe = [&](ll x,ll y) {
    ...
    while(not q.empty())
    {
        ...
        fori(0,t)
        {
            pii u = q.front();
            q.pop();
            if(issafe(u.fs,u.sc+1))
            {
                if(s[u.fs][u.sc+1]<mn)
                {
                    se.clear();
                    se.insert({u.fs,u.sc+1});
                    mn = s[u.fs][u.sc+1];
                }
                else if(s[u.fs][u.sc+1]==mn)
                {
                    se.insert({u.fs,u.sc+1});
                }
            }
            if(issafe(u.fs+1,u.sc))
            {
                if(s[u.fs+1][u.sc]<mn)
                {
                    se.clear();
                    se.insert({u.fs+1,u.sc});
                    mn = s[u.fs+1][u.sc];
                }
                else if(s[u.fs+1][u.sc]==mn)
                {
                    se.insert({u.fs+1,u.sc});
                }
            }
