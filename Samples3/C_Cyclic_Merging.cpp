...
int main()
{
    ...
    while(t--)
    {
        ...
        min_heap<pii> pq;
        fori(0,n-1)
            pq.push({max(arr[i],arr[i+1]),i});
        pq.push({max(arr.front(),arr.back()),n-1});
        ll ans=0;
        while(not pq.empty())
        {
            auto [u,idx] = pq.top();
            pq.pop();
            if(not d.unite(idx,((idx+1<=n-1)? idx+1:0)))
                continue;
            ans += u;
            if(idx<n-2)
                pq.push({max(u,arr[idx+2]),idx+1});
            else if(idx==n-2)
                pq.push({max(u,arr[0]),n-1});
            else
                pq.push({max(u,arr[1]),0});
            if(idx>=1)
                pq.push({max(u,arr[idx-1]),idx-1});
            else
                pq.push({max(u,arr.back()),n-1});
        }
        cout<<ans;
        nl;
    }
