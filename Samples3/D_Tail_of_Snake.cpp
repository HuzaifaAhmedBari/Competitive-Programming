...
int main()
{
    ...
    vl a1(n), a2(n), a3(n);
    cin>>a1>>a2>>a3;
    vl p1(n+1,0), p2(n+1,0), p3(n+1,0);
    partial_sum(all(a1),p1.begin()+1);
    partial_sum(all(a2),p2.begin()+1);
    partial_sum(all(a3),p3.begin()+1);
    ll ans = LLONG_MIN, mx = LLONG_MIN;
    fori(2,n)
    {
        mx = max(mx,p1[i-1]-p2[i-1]);
        ans = max(ans,mx+p2[i]-p3[i]);
    }
    cout<<ans+p3[n];
    nl;
    return 0;
