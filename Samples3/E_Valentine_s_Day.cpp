...
int main()
{
    ...
    fori(0,n)
    {
        swap(arr1[i].fs,arr1[i].sc);
    }
    fori(0,m)
    {
        arr2[i].fs--;
        arr2[i].sc--;
        s[arr2[i].fs][arr2[i].sc] = true;
        s[arr2[i].sc][arr2[i].fs] = true;;
    }
    ll ans = 0;
    fori(0,n)
    {
        ans = max(ans,arr1[i].fs);
    }
    fori(0,n)
    {
        forj(0,n)
    ...
    fori(0,n)
    {
        forj(0,n)
        {
            ...
            fork(0,n)
            {
                if(j==k or i==k)
                    continue;
                if(s[i][j] or s[i][k] or s[j][i] or s[j][k] or s[k][i] or s[k][j])
                    continue;
                ll x = arr1[i].fs;
                ll z = (x*arr1[j].sc)+arr1[j].fs;
                ll y = (z*arr1[k].sc)+arr1[k].fs;
                ans = max(ans,y);
            }
        }
    }
