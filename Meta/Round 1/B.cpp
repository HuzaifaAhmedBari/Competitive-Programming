...
int main()
{
    ...
    fork(1,t+1)
    {
        ...
        ll l=0,r=(1e9)+1;
        ll ans=0;
        function<bool(ll,ll,ll,vb&)> f = [&](ll x,ll z,ll mid,vb &visited) {
            return (x>=0 and x<n and not visited[x] and abs(arr[z]-arr[x])<=mid);
        };
        function<bool(ll)>fine = [&](ll x) {
            queue<ll> q;
            vb visited(n,false);
            fori(0,n)
            {
