...
int main()
{
    ...
    for(ll z=1;z<=t;z++)
    {
        ...
        fori(0,m)
        {
            ...
            {
                cin>>temp[j];
                idx[temp[j]].push_back({i,j});
            }
            visited[i].assign(x,false);
        ...
        for(auto [x,y]:idx[1])
        {
            ...
            fori(1,k+1)
            {
                if(y+i>=routes[x].size())
                    break;
                q.push({routes[x][y+i],1LL});
                visited[x][y+i] = true;
            }
        }
        while(not q.empty())
            ...
            for(auto [x,y]:idx[u])
            {
                ...
                fori(1,k+1)
                {
                    ...
                    if(visited[x][y+i])
                        break;
                    visited[x][y+i] = true;
                    if(c+1<dist[routes[x][y+i]] or dist[routes[x][y+i]]==-1)
                        q.push({routes[x][y+i],c+1});
                }
            }
        }
        ll ans = 0;
